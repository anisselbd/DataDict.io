...
export default function ProjectViewer({ initialProject }) {
    const params = useParams();
    const slug = params.slug;

    const [project, setProject] = useState(initialProject || null);
    const [selectedEntityId, setSelectedEntityId] = useState(initialProject?.entities?.[0]?.id || null);
    const [loading, setLoading] = useState(!initialProject);
    const [error, setError] = useState(initialProject ? '' : '');
    const [copied, setCopied] = useState(false);

    const loadData = useCallback(async () => {
        if (!slug || initialProject) return;
        try {
            setLoading(true);
