...
let ExportController = class ExportController {
    ...
    async exportMarkdown(user, id, res) {
        const markdown = await this.exportService.exportToMarkdown(id, user.id);
        res.setHeader('Content-Type', 'text/markdown; charset=utf-8');
        res.setHeader('Content-Disposition', `attachment; filename="data-dictionary-${id}.md"`);
        res.send(markdown);
    }
    async exportPdf(user, id, res) {
        const pdfBuffer = await this.exportService.exportToPdf(id, user.id);
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="data-dictionary-${id}.pdf"`);
        res.send(pdfBuffer);
    }
};
